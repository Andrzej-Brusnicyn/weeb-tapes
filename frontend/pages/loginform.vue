      ...
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form @submit.prevent="login" class="space-y-4 md:space-y-6" action="#">
            ...
            <button type="submit" class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 bg-red-600">Sign in</button>
            <p class="text-sm font-light text-gray-500 dark:text-gray-400">
              Don’t have an account yet? <NuxtLink to="/registrform" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign up</NuxtLink>
            </p>
...
const login = async () => {
    const response = await fetch('http://api.weeb-tapes.de/api/login', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      error.value = errorData.errors ? errorData.errors.message.join(', ') : 'Login failed'
      return
    }
